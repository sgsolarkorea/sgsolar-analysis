...
function Chart({
  ...
  return (
    <div id="generation">
      <div>
        <p className="text-[13px] font-semibold text-slate-500">예상 연간 발전량</p>
        <p className="mt-1 text-[48px] font-extrabold tracking-tight text-navy sm:text-[56px]">
      ...
      <div className="relative mt-8">
        <div className="relative grid h-[300px] grid-cols-12 items-end gap-1.5 sm:h-[380px] sm:gap-2">
          <div
          ...
          {data.map((d) => {
            ...
            return (
              ...
              >
                {isPeak ? (
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap text-[11px] font-bold text-sky-600">
                    {d.month}
                  </span>
                ) : null}
                <div
                  className={`w-full rounded-t-sm ${isPeak ? "bg-sky-500" : "bg-navy/85"}`}
        ...
        <div className="mt-2 grid grid-cols-12 gap-1.5 sm:gap-2">
          {data.map((d) => (
            <span key={`lbl-${d.month}`} className="text-center text-[11px] font-medium text-slate-500">
              {d.month.replace("월", "")}
            </span>
          ))}
        </div>
